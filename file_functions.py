...
# add child directory/file to existing path string
def join_str(listofstr):
    if all(map(lambda x: isinstance(x, str), listofstr)):
...
# SAVE TO PKL (fullpath)
def savetopkl_fullpath(fullpath, data):
    with open(fullpath, "wb") as targetfile:
        pkl.dump(data, targetfile, protocol=4)


# SAVE TO DF to CSV
def savedftocsv_fullpath(fullpath, data):
    data.to_csv(index=False, path_or_buf=fullpath)
