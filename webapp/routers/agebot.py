...
def calc_agebot(
    ...
    if agestat_data and trialstat_data and callback_context.triggered[0]['prop_id'].endswith('sort_by'):
        ...
        statdf = DataTableOperations().sort_datatable(agestat_sort, statdf)
        alltrialsummariesdf = pd.DataFrame.from_records(trialstat_data)
        alltrialsummariesdf = DataTableOperations().sort_datatable(trialstat_sort, alltrialsummariesdf)
