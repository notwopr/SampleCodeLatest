...
from file_functions import delete_folder, getbotsinglerunfolder, readpkl
...
def get_maxinvestperiod(num_periods, start_date):
    if num_periods and start_date:
        maxip = DateOperations().num_days_string(start_date, staticmaxdate) // num_periods
...
def get_maxnumpp(period, start_date):
    if period and start_date:
        maxnum = DateOperations().num_days_string(start_date, staticmaxdate) // period
...
def get_maxdatep(period, num_periods):
    if period and num_periods:
        newmaxdate = DateOperations().plusminusdays(staticmaxdate, (period * num_periods), 'subtract')
...
def run_strattester(n_clicks, strat, period, num_periods, start_date, min_age, benchmark, startcapital, rankstart, rankend, activecell):
    ...
    elif activecell and os.path.exists(Path(DirPaths().strattester_allperiodstatdf) / f'strattester_allperiodstatdf_{activecell}.pkl'):
        reply = html.P(f'Viewing archived test run {activecell}')
        testrundict = readpkl(f'strattester_testrun_{activecell}', Path(DirPaths().strattester_testruns))
        finalreport = strattest_sequential_final_report(testrundict)
        allperiodstatdf = readpkl(f'strattester_allperiodstatdf_{activecell}', Path(DirPaths().strattester_allperiodstatdf))
        stockperfdfreports = readpkl(f'strattester_stockperfreports_{activecell}', Path(DirPaths().strattester_stockperfreports))
        periodreports = gen_periodreports(stockperfdfreports, bp.botid)
    elif activecell:
