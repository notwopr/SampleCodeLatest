'''
daily_growthrate = a given period growth rate converted to a daily growth rate via geometric rate formula conversion
daily_growthrate_min = out of all the period growth rates in a test run (converted to daily rates), this is the minimum rate in that group
margin_daily_growthrate_min = daily_growthrate_min of portfolio over the same for the benchmark in a given testrun
'''
# IMPORT TOOLS
...
def savetestrun(bp, allperiodstatdf, num_periods, runtime):
    ...
    savetopkl(f'strattester_testrun_{bp["todaysdate"]+bp["testnumber"]}', Path(DirPaths().strattester_testruns), testrundict)
    return testrundict


def gen_leaderboard(colconfig):
    leaderdf = pd.DataFrame(data=[readpkl_fullpath(child) for child in Path(DirPaths().strattester_testruns).iterdir()])
