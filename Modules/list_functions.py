...
# RETURNS LIST OF ITEMS THAT HAVE DUPLICATES
def find_duplicates(dupelist):
    dupes = []
    i = 0
    while i < len(dupelist)-1:
        end = i+1
        if dupelist[i] == dupelist[end]:
            dupes.append(dupelist[i])
        while dupelist[i] == dupelist[end] and end < len(dupelist):
            end += 1
        i = end
    return dupes
