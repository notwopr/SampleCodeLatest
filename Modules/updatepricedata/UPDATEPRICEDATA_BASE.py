...
1.3: Update for webapp use.  use savetopkl function, change multiprocessorshell to multiprocessorshell_mapasync
"""
...
from Modules.updatepricedata.UPDATEPRICEDATA_UPDATEINDEXPRICEFILE import indexpriceupdater
from Modules.updatepricedata.UPDATEPRICEDATA_TIINGO import stockpriceretrieval, marketcapretrieval, fundamentalretrieval
from Modules.multiprocessing import multiprocessorshell_mapasync
...
def download_prices(targetfolder, start, end, symbol):
    if symbol in ["^DJI", "^INX", "^IXIC"]:
        ...
        try:
            ...
            prices["date"] = prices["date"].apply(dt.datetime.date)
        # SAVE TO FILE
        savetopkl(f"{symbol}_prices", targetfolder, prices)
...
def store_allprices(destfolder, tickerfile, option, chunksize):
    ...
    if option == "benchmark":
        ...
        symbols = readpkl_fullpath(tickerfile)['symbol'].tolist()
    # SET DATE RANGE
    start = "1962-01-01"
    ...
    multiprocessorshell_mapasync(destfolder, download_prices, symbols, 'no', (destfolder, start, end), chunksize)
...
def download_marketcap(targetfolder, start, end, symbol):
    try:
        ...
        marketcapdf["date"] = marketcapdf["date"].apply(dt.datetime.date)
    # SAVE TO FILE
    savetopkl(f"{symbol}_marketcaps", targetfolder, marketcapdf)
...
def store_allmarketcap(destfolder, tickerfile, chunksize):
    ...
    # DOWNLOAD
    multiprocessorshell_mapasync(destfolder, download_marketcap, symbols, 'no', (destfolder, start, end), chunksize)
...
def download_fundamentals(targetfolder, start, end, symbol):
    try:
        ...
        fundydf["date"] = fundydf["date"].apply(dt.datetime.date)
    # SAVE TO FILE
    savetopkl(f"{symbol}_fundies", targetfolder, fundydf)
...
def store_allfundies(destfolder, tickerfile, chunksize):
    ...
    # DOWNLOAD
    multiprocessorshell_mapasync(destfolder, download_fundamentals, symbols, 'no', (destfolder, start, end), chunksize)
