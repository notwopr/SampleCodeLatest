...
def comp_edge_needed(globalparams):
    ...
    return [
        ...
        f'SWITCH_COST: ${formalnumber(comp_fund.client_actualgain * switch_factor)}',
        f"{your_fund.profile} experienced a growth rate of {formalnumber(your_fund.fund_perf*100)} % (corresponding client profit of ${formalnumber(your_fund.client_actualgain)}) while {comp_fund.profile} experienced a growth rate of {formalnumber(comp_fund.fund_perf*100)} % (corresponding client profit of ${formalnumber(comp_fund.client_actualgain)}).  Had client gone with {your_fund.profile} instead of {comp_fund.profile}, they would experience a swing of ${formalnumber(edge_earnings)}.",
        f"WOULD THE CLIENT SWITCH OVER TO {your_fund.profile} IN THESE CIRCUMSTANCES?",
        f"The client would want to switch over to {your_fund.profile} if the client would have made more money going with {your_fund.profile} than with {comp_fund.profile}.  Specifically, the client's actual gain (the amount the client earns after fees) with {your_fund.profile} must be greater than the sum of the client's actual gain with {comp_fund.profile} and the cost for the client to switch over, which we represent here as a proportion of the client's actual gain with {comp_fund.profile} (called the switch_factor).  In this case, the switch_factor is {formalnumber(switch_factor*100)} %, and the client's actual gain with {comp_fund.profile} was ${formalnumber(comp_fund.client_actualgain)}, which corresponds to a switch cost of ${formalnumber(comp_fund.client_actualgain * switch_factor)}.  Therefore, for the client to switch over to {your_fund.profile}, {your_fund.profile} would need to produce for the client an actual gain greater than ${formalnumber(comp_fund.client_actualgain+abs(comp_fund.client_actualgain*switch_factor))} (${formalnumber(comp_fund.client_actualgain)} + ${formalnumber(comp_fund.client_actualgain * switch_factor)}).",
