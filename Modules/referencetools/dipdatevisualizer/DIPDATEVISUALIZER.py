...
def dipdate_report(stock, beg_date, end_date, peakdate, valleydate, peakprice, valleyprice, recoverydate):
    dipdatereport = f"Between {beg_date} and {end_date}, {stock}'s largest drop in price spanned a total of {formalnumber_integer(DateOperations().num_days_string(peakdate, valleydate))} days ({peakdate} to {valleydate}), during which time it dropped {formalnumber(((valleyprice-peakprice)/peakprice)*100)} %, falling ${formalnumber(valleyprice-peakprice)} per share, from ${formalnumber(peakprice)} down to ${formalnumber(valleyprice)}."
    if recoverydate is None:
        ...
        recoveryreport = f"  {stock} recovered its peak price of ${formalnumber(peakprice)} per share within the date range. It took {formalnumber_integer(DateOperations().num_days_string(valleydate, recoverydate))} days to recover its peak price (from {valleydate} (the date of its lowest price) to {recoverydate})."
