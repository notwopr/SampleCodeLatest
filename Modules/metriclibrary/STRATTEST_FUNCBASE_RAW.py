...
from Modules.metriclibrary.STRATTEST_FUNCBASE import cleanchanges
from Modules.growthcalcbot import removeleadingzeroprices


# get global min/max/first/last price
def globalpricegrab_single(prices, stock, pricetype):
    if pricetype == 'min':
        targetprice = prices[stock].min()
    elif pricetype == 'max':
        targetprice = prices[stock].max()
    elif pricetype == 'first':
        targetprice = prices[stock].iloc[0]
    elif pricetype == 'last':
        targetprice = prices[stock].iloc[-1]
    return targetprice
...
# get pct difference between current price and most recent ATH
def currtoathdiff_single(prices, stock):
    # get current price
    currprice = globalpricegrab_single(prices, stock, 'last')
    # get current ATH
    currath = globalpricegrab_single(prices, stock, 'max')
    # get difference
    return (currprice - currath) / currath
...
def getpctchange_single(prices, focuscol):
    firstp = prices[focuscol].iloc[0]
    lastp = prices[focuscol].iloc[-1]
    # if first price is zero, get first nonzero price
    i = 1
    while firstp == 0 and i < len(prices):
        firstp = prices[focuscol].iloc[i]
        i += 1
    if firstp != 0:
        pctchange = (lastp - firstp) / firstp
    else:
        pctchange = 0
    return pctchange
